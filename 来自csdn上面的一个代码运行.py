"""
文章来源：https://blog.csdn.net/chengxuyuanyonghu/article/details/59716405
官方说明：https://docs.python.org/2/library/argparse.html#argument-abbreviations-prefix-matching
一、简介：
...
三、方法ArgumentParser(prog=None, usage=None,description=None, epilog=None, parents=[],formatter_class=argparse.HelpFormatter,
prefix_chars='-',fromfile_prefix_chars=None, argument_default=None,conflict_handler='error', add_help=True)
这些参数都有默认值，当调用parser.print_help()或者运行程序时由于参数不正确(此时python解释器其实也是调用了print_help()方法)时，
会打印这些描述信息，一般只需要传递description参数，
如上。
